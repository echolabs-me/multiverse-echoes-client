...
export function useTickTimer(): TickTimerData {
  ...
  const arrivedTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );

  const handleWsEvent = useCallback((event: WsEchoEvent) => {
  ...
  useEffect(() => {
    const compute = () => {
      ...
      if (stateRef.current === 'arrived' || stateRef.current === 'waiting')
        return;
      ...
      if (remaining <= 0) {
        ...
        if (
          ...
          const generatingFor =
            (Date.now() - generatingStartRef.current) / 1000;
          if (generatingFor > 90) {
            const now = Date.now();
            lastTickAtRef.current = now;
            writeLastTickAt(now);
        ...
        if (
          stateRef.current !== 'counting_down' &&
          stateRef.current !== 'generating'
        )
          return;
