...
export function CommunityPulseCard() {
  ...
  const fetchFeed = useCallback(() => {
    void feeds
      .personal()
      .then((page) => setItems(page.data.slice(0, 12)))
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="flex flex-col gap-1">
      {display.map((item) => {
        ...
        const isLifeEvent =
          item.item_type === 'life_event' || item.item_type === 'LifeEvent';
        const isRelationship =
          item.item_type === 'relationship_change' ||
          item.item_type === 'RelationshipChange';
        ...
        return (
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <Icon
                size={11}
                className={`shrink-0 ${iconColor} ${isLifeEvent ? 'fill-accent' : ''}`}
              />
              ...
              <span className="ms-auto text-[10px] whitespace-nowrap text-text-muted">
                {(() => {
                  ...
                  if (minutes < 60)
                    return t('communityFeed.minutesAgo', { count: minutes });
                  const hours = Math.floor(minutes / 60);
                  if (hours < 24)
                    return t('communityFeed.hoursAgo', { count: hours });
