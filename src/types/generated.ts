 ...
 */
export type AdminEnforcementActionTarget =
 ...
 *  toast.
 */
export type AdminTriggerRevenueSnapshotResponse =
  | { outcome: 'inserted'; snapshot: AdminRevenueSnapshotItem }
  | { outcome: 'already_exists'; period_start: string };
...
export type EchoRelationshipType =
  | 'Friend'
  | 'Rival'
  | 'RomanticPartner'
  | 'Colleague'
 ...
 */
export type EnforcementActionTargetFilter =
  | { User: { user_id: string | null } }
  | { Echo: { echo_id: string | null } }
  | 'Any';
...
export type WorldEventPayload =
  | { EchoCreated: { echo_id: string; owner_id: string } }
  | { EchoDeleted: { echo_id: string } }
  | { EchoHibernated: { echo_id: string; reason: string } }
  | { EchoWoken: { echo_id: string } }
  /**
  ...
  | { TickStarted: { tick_id: number } }
  | { TickCompleted: { tick_id: number } }
  | { DiaryEntryGenerated: { echo_id: string; diary_id: string } }
  | {
      DiaryImageReady: { echo_id: string; diary_id: string; image_url: string };
    }
  | { LifeEventOccurred: { echo_id: string; event_id: string } }
  | { MoodChanged: { echo_id: string; old_mood: string; new_mood: string } }
  | {
      EchoInteraction: {
        echo_a: string;
        echo_b: string;
  ...
  | { RelationshipChanged: { echo_a: string; echo_b: string } }
  | { ShardCreated: { shard_id: string; shard_type: string; owner_id: string } }
  | {
      ShardStatusChanged: {
        shard_id: string;
        old_status: string;
   ...
   *  (ME-SDB-001 §9.2).
   */
  | { ShardAdminFlagUnflagged: { shard_id: string; admin_id: string } }
  | {
      ShardTravelRequested: {
        echo_id: string;
        from_shard: string;
        to_shard: string;
      };
    }
  | { ShardTravelApproved: { echo_id: string; destination_shard: string } }
  | { ShardTravelCompleted: { echo_id: string; shard_id: string } }
  | { ShardTravelDenied: { echo_id: string; reason: string } }
  | { ShardCapacityWarning: { shard_id: string; percent: number } }
  | {
      EchoMoved: {
        echo_id: string;
        shard_id: string;
  ...
  | { GlobalEventPropagated: { event_id: string; affected_shards: string[] } }
  ...
  | { InfluencePointUsed: { user_id: string; echo_id: string; points: number } }
  ...
  | { PersonaUpdated: { echo_id: string; version: number } }
  | { ConversationSaved: { echo_id: string; user_id: string } }
  // Phase 7 — user feedback system.
  | { FeedbackSubmitted: { user_id: string; feedback_id: string } }
  | {
  ...
  | { UserFollowed: { source_user_id: string; target_user_id: string } }
  | { UserBlocked: { source_user_id: string; target_user_id: string } }
  | { UserMuted: { source_user_id: string; target_user_id: string } }
  /**
  ...
  | { NotificationCreated: { user_id: string; notification_id: string } }
  ...
  | { EchoAvatarReady: { echo_id: string; avatar_url: string } }
  | { EmergencyHibernation: { reason: string } }
  /**
   ...
   */
  | {
      PromotionalSubscriptionExpiring: {
        user_id: string;
