...
import { Card, Badge, Spinner, EmptyState } from '../components/index.ts';
                  ...
                  <div className="flex flex-col gap-2 p-4">
                    <div className="flex items-start justify-between">
                      <h3 className="text-sm font-semibold text-text-primary">
                        {t(
                          `shardNames.${shard.name.toLowerCase().replace(/\s+/g, '-')}`,
                          { defaultValue: shard.name },
                        )}
                      </h3>
                      <Badge
                      ...
                      >
                        {t(`shardBrowser.status${shard.status}`, {
                          defaultValue: shard.status,
                        })}
                      </Badge>
                    </div>
                    <p className="line-clamp-4 text-xs text-text-secondary">
                      {t(
                        `shardDescriptions.${shard.name.toLowerCase().replace(/\s+/g, '-')}`,
                        { defaultValue: shard.description },
                      )}
                    </p>
                    ...
                    <div className="flex items-center gap-2 text-xs text-text-muted">
                      ...
                      <span>
                        {t(`shardBrowser.type${shard.shard_type}`, {
                          defaultValue: shard.shard_type,
                        })}
                      </span>
                      <span>&middot;</span>
                      <span>
                        {t('shardBrowser.echoCount', {
                          current: shard.current_active_count,
                          max: shard.max_active_echoes,
                        })}
                      </span>
                    </div>
                  </div>
