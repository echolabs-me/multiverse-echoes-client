...
export function EchoDetailPage() {
  ...
  const { activeEcho, fetchEcho, hibernateEcho, wakeEcho, deleteEcho } =
    useEchoStore();
  ...
  const fetchEchoesTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const convList = await conversations
        .list(echoId)
        .catch(() => [] as Conversation[]);
      setPastConversations(convList);
      // Load solo_mode
      const privacy = await accountApi
        .getPrivacy()
        .catch(() => ({ solo_mode: false }));
  ...
  const loadMoreDiary = useCallback(async () => {
    ...
    try {
      ...
      const page = await echoApi.diary(
        echoId,
        PAGE_SIZE,
        diaryCursor,
        activeMood,
  ...
  useEffect(() => {
    ...
    const limit = activeMood
      ? PAGE_SIZE
      : Math.max(PAGE_SIZE, maxLoadedRef.current);
    void echoApi
      .diary(echoId, limit, undefined, activeMood)
      .then((page) => {
    ...
    (event: WsEchoEvent) => {
      ...
      if (event.type === 'ConnectionEstablished') {
        if ('last_tick_at' in event) {
          ...
          if (serverAt > 0) {
            try {
              localStorage.setItem('me_last_tick_at', String(serverAt));
            } catch {
              /* noop */
            }
          }
          ...
          void echoApi
            ...
            .then((page) => {
              ...
              const arrivals = entries.filter(
                (e) => !knownDiaryIdsRef.current.has(e.diary_id),
              );
              if (arrivals.length > 0) {
                const arrivalIds = new Set(arrivals.map((e) => e.diary_id));
                setNewDiaryIds(
                  (prev) => new Set<string>([...prev, ...arrivalIds]),
                );
              }
  ...
  const handleUseInfluence = async () => {
    ...
    try {
      ...
      trackEvent('nudge.sent', {
        echo_id: activeEcho.echo_id,
        influence_type: influenceType,
      });
  ...
  return (
    <div className="flex h-full">
      ...
      <div
        ...
        className={`relative flex h-full flex-col ${showConversation ? 'w-[60%]' : 'w-full'} transition-[width] duration-300`}
      >
          ...
          <div className="mx-auto max-w-4xl p-6">
            ...
            <div className="mbe-6 flex items-start gap-4">
              {activeEcho.avatar_url ? (
                ...
                >
                  <EchoPortrait3D
                    name={activeEcho.name}
                    mood={activeEcho.current_mood}
                    size="lg"
            ...
            <div className="my-4 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <div className="relative flex-1">
                  ...
                  {nudgeRipple && (
                    <span
                      className="pointer-events-none absolute inset-0 animate-nudge-ripple rounded-xl"
                      aria-hidden="true"
                    />
                  )}
                </div>
                  ...
                  <>
                    ...
                    <div className="absolute inset-s-0 z-50 mbs-1 w-48 rounded-lg border border-border bg-surface py-1 shadow-lg">
                      ...
                      >
                        ...
                        {t('echoDetail.exportStory')}
                      </button>
                      <div
                        className="my-1 border-bs border-border"
                        role="separator"
                      />
                      <button
                        onClick={() => {
            ...
            {pastConversations.length > 0 && (
              <div className="mbe-4">
                ...
                >
                  ...
                  {t('conversation.pastConversations', {
                    name: activeEcho.name,
            ...
            {activeEcho &&
              ...
              (() => {
                const shardName =
                  shardList.find(
                    (s) => s.shard_id === activeEcho.current_shard_id,
                  )?.name ?? activeEcho.current_shard_id;
                return (
              ...
              {(diaryEntries.length > 0 || moodFilter || diarySearch) && (
                <div className="mbe-3 flex flex-wrap items-center gap-2">
                  <div className="relative min-w-45 flex-1">
                    <Search
                      size={14}
                      className="absolute inset-s-2.5 inset-bs-1/2 -translate-y-1/2 text-text-muted"
                      aria-hidden="true"
                    />
                    <input
                      type="text"
              ...
              {diaryEntries.length === 0 && !moodFilter && !diarySearch ? (
                ...
                <div className="flex flex-col gap-2">
                  {diaryByDay.map(({ day, entries }) => {
                    ...
                    const expanded = toggledDays.has(day)
                      ? !defaultExpanded
                      : defaultExpanded;
                    return (
                      <div key={day}>
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-xs text-accent">
                              {t(`relationships.${rel.relationship_type}`, {
                          ...
                          <Badge
                            variant={
                              rel.status === 'Active' ? 'success' : 'default'
                            }
                          >
                            {t(`relationships.${rel.status}`, {
                              defaultValue: rel.status,
                            })}
                          </Badge>
                        </div>
                        <div className="mbs-2">
                            ...
                            <div
                              ref={(el) => {
                                if (el)
                                  el.style.setProperty(
                                    '--me-progress',
                                    `${Math.min(100, Math.max(0, sentimentPct))}%`,
                                  );
                              }}
        ...
        >
          ...
          <div className="mbe-4 flex flex-col gap-3">
            ...
            >
              ...
              <option value="suggest_activity">
                {t('echoDetail.influenceSuggest')}
              </option>
              <option value="hint_relationship">
                {t('echoDetail.influenceInspire')}
              </option>
            </select>
            <Input
...
function DiaryCard({
  ...
  const handleWatchNarration = useCallback(async () => {
    ...
    try {
      ...
      const startResult = await echoApi.narrateVideoStart(
        entry.echo_id,
        entry.diary_id,
      );
      ...
      const poll = async (): Promise<void> => {
        while (attempts < maxAttempts) {
          ...
          const status = await echoApi.narrateVideoStatus(
            entry.echo_id,
            entry.diary_id,
            jobId,
          );
          ...
          if (status.status === 'complete') {
            ...
            for (let fetchAttempt = 0; fetchAttempt < 3; fetchAttempt++) {
              try {
                ...
                videoBlob = await echoApi.narrateVideoResult(
                  entry.echo_id,
                  entry.diary_id,
                  jobId,
                );
  ...
  return (
    ...
    >
      <Card
        ...
        onAnimationEnd={
          isNew ? () => onAnimationEnd?.(entry.diary_id) : undefined
        }
      >
        {entry.image_url && (
          <img
